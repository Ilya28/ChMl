...
import static chml.MainWindow.redrawFast;
...
import static chml.MainWindow.RFP;
import java.awt.*;
        ...
        if (redrawAll) {
                ...
                for (int j = 0; j < this.getHeight(); j++) {
                    switch (DM) {
                        ...
                        case 3:
                            ...
                            if (field[i][j] > 0) {
                                kf = 5 * CMV / maxq;
                                if ((1 - (field[i][j] / maxf) > 1 / (kf * field[i][j])) || (field[i][j] / maxf < 0.5)) {                      //if ((255 - 255 * (field[i][j] / maxf) > 255 / (kf * field[i][j]))||(0.5 - (field[i][j] / maxf) > 0)) {
                                    pc = new Color(255, max255((int) (255 - 255 * (field[i][j] / maxf))), max255((int) (255 - 255 * (field[i][j] / maxf))));
                                } else {
                                    pc = new Color(255, max255((int) (255 / (kf * field[i][j]))), max255((int) (255 / (kf * field[i][j]))));
                                }
                                ...
                                kf = -5 * CMV / minq;
                                if ((1 - (field[i][j] / minf) >  1 / (kf * field[i][j])) && (0.5 - (field[i][j] / minf) > 0)) {
                                    pc = new Color(max255((int) (255 - 255 * (field[i][j] / minf))), max255((int) (255 - 255 * (field[i][j] / minf))), 255);
                                } else {
                                    pc = new Color(max255((int) (-255 / (kf * field[i][j]))), max255((int) (-255 / (kf * field[i][j]))), 255);
                                }
            ...
            for (int k = 0; k < Charges.count; k++) {
               ...
               // ==============================   FAST     ============================
                //        ==============================================================
            redrawFast = false;    
            double[][] field = new double[this.getWidth()/RFP][];
            double[][] vfieldx = new double[this.getWidth()/RFP][];
            double[][] vfieldy = new double[this.getWidth()/RFP][];
            for (int i = 0; i < field.length; i++) {
                field[i] = new double[this.getHeight()/RFP];
                vfieldx[i] = new double[this.getHeight()/RFP];
                vfieldy[i] = new double[this.getHeight()/RFP];
            }
            for (int i = 0; i < this.getWidth()/RFP; i++) {
                for (int j = 0; j < this.getHeight()/RFP; j++) {
                    field[i][j] = 0;
                    vfieldx[i][j] = 0;
                    vfieldy[i][j] = 0;
                    //double e = 0;
                    if (FT == 1) {
                        for (int k = 0; k < Charges.count; k++) {
                            int x = i - Charges.arr[k].x/RFP;
                            int y = j - Charges.arr[k].y/RFP;
                            vfieldx[i][j] += (x * Charges.arr[k].v) / Math.pow(x * x + y * y, 1.5);
                            vfieldy[i][j] += (y * Charges.arr[k].v) / Math.pow(x * x + y * y, 1.5);
                            field[i][j] += 50 * Charges.arr[k].v / (x * x + y * y);
                        }
                        ...
                        for (int k = 0; k < Charges.count; k++) {
                            field[i][j] += Charges.arr[k].v / (Math.sqrt(sq((i - Charges.arr[k].x)/RFP) + sq((j - Charges.arr[k].y)/RFP)));
                        }
                    }
                    if (field[i][j] > maxf) {
                        maxf = field[i][j];
                ...
                for (int j = 0; j < this.getHeight()/RFP; j++) {
                    switch (DM) {
                        case 1:
                            pc = new Color(230, 255, 230);
                            if (field[i][j] > 0) {
                                kf = 5 * CMV / (maxq*RFP*RFP);
                                pc = new Color(255, max255((int) (255 / (kf * field[i][j]))), max255((int) (255 / (kf * field[i][j]))));
                            } else if (field[i][j] < 0) {
                                kf = -5 * CMV / (minq*RFP*RFP);
                                pc = new Color(max255((int) (-255 / (kf * field[i][j]))), max255((int) (-255 / (kf * field[i][j]))), 255);
                            }
                            break;
                        case 2:
                            maxf = RFP*RFP*maxq / CMV;
                            minf = RFP*RFP*minq / CMV;
                            if (field[i][j] > 0) {
                                pc = new Color(255, max255((int) (255 - 255 * (field[i][j] / maxf))), max255((int) (255 - 255 * (field[i][j] / maxf))));
                            } else if (field[i][j] < 0) {
                                pc = new Color(max255((int) (255 - 255 * (field[i][j] / minf))), max255((int) (255 - 255 * (field[i][j] / minf))), 255);
                    ...
                    g2d.setColor(pc);
                    g2d.fillRect(RFP*i, RFP*j, RFP, RFP);
                }
            }
            // ---------------------------------------------------
            ...
            for (int k = 0; k < Charges.count; k++) {
                ...
                for (int i = 0; i < 24; i++) {
                    ...
                    int kx, ky;
                    int dkx, dky;
                    int x = Charges.arr[k].x/RFP;
                    int y = Charges.arr[k].y/RFP;
                    //g2d.setColor(Color.CYAN);
                    //g2d.fillRect(x + (int)px, y + (int)py, 3, 3);
                    g2d.setColor(Color.BLACK);
                    double kof = 0.05;
                    ...
                    for (int j = 0; j < 10000/RFP; j++) {
                        kx = x + (int) px;
                        ky = y + (int) py;
                        dkx = (int) ((x + px)*RFP);
                        dky = (int) ((y + py)*RFP);
                        if ((kx > 0) && (kx < this.getWidth()/RFP) && (ky > 0) && (ky < this.getHeight()/RFP)) {
                            npx = px + kof * vfieldx[kx][ky] / Math.sqrt(sq(vfieldx[kx][ky]) + sq(vfieldy[kx][ky]));
                            npy = py + kof * vfieldy[kx][ky] / Math.sqrt(sq(vfieldx[kx][ky]) + sq(vfieldy[kx][ky]));
                        } else {
                            break;
                        }
                        //if (((int) px*RFP != (int) npx*RFP) || ((int) py*RFP != (int) npy*RFP)) {
                           // if (Charges.isCTC(dkx, dky, 2)) {
                           //     break;
                           // }
                            Buf.setRGB(dkx, dky, 0);
                        //}
                        //g2d.drawLine(x + (int)px, y + (int)py, x + (int)npx, y + (int)npy);
                        px = npx;
                        py = npy;
